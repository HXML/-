<template>
  <div class="TemplateTow">
    <div class="banner">
      <van-swipe class="my-swipe" :autoplay="3000" indicator-color="white">
        <van-swipe-item v-for="(item,index) in BannerList" :key="index" @click="bannerhref(item.linkUrl)">
          <img :src="item.imgStr+'?x-oss-process=style/b4'" alt />
        </van-swipe-item>
      </van-swipe>
    </div>
    ...
    >
      <div class="OneContent">
        <div class="OneContent_item" v-for="(item,index) in TemplateTowList" :key="index"  @click="jumpGoodsDetails(item.id)">
          <div :class="index%3==0?'class1':'class2'">
            <div class="top">
              <img :src="item.mainImg+'?x-oss-process=style/b4'" alt />
            </div>
            <div class="bottom">
...
export default {
  ...
  methods: {
    ...
    // 获取首页轮播图
    ZkShopBannerData() {
      // console.log(window.localStorage.getItem('ShopId'),"ShopId")
      let data = {
    ...
    GetbusgoodsByShoplist() {
      ...
      GetbusgoodsByShoplistAPI(this.shoplist, "get")
        ...
        .then(res => {
          ...
          // console.log(this.TemplateTowList.length, this.totalTow);
