...
import { GetPwdChangeAPI } from "@/api/login";
import qs from "qs";
export default {
  name: "LoginResetpassword",
  data() {
    return {
      titledata: "重置密码", //控制title的动态变量
  ...
  methods: {
    ...
    confirm() {
      let data = qs.stringify({
        ...
        pwd: this.myinput1value,
        confirmPwd: this.myinput2value
      });
      GetPwdChangeAPI(data, "post").then(res => {
        console.log(res, "确认修改密码");
        if (res.code == 1) {
            Toast("密码修改成功");
            this.$router.replace({ name: "LoginEnter" });
        } else {
