    ...
    <div class="ApplyRefund_content">
      ...
      <div class="amount">
        <p class="p1">
          ...
          <input type="number" class="s3" v-model="submitdata.ReturnPrice" min="0" />
          <!-- <span class="s3">￥{{ShopDetails.totalPrice}}.00</span> -->
        </p>
        <p class="p2">*可修改，退款金额不包含邮费，最多退款￥{{ShopDetails.totalPrice}}</p>
      </div>
  ...
  methods: {
    ...
    submit() {
      let data = qs.stringify({
        ...
        VoucherImgId: this.fileListTow.join(','),
