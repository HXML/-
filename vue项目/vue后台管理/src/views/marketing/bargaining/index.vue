  ...
  <div class="activeList" v-else>
      ...
      <div class="flex">
        ...
        <section class="flex aic ml-40">
          ...
          <el-input
            class="w-160"
            placeholder="请输入商品名称"
            v-model="queryListData.goodsName"
          ></el-input>
        </section>
        <section class="flex aic ml-40">
          <span>有效期：</span>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column
        ...
        label="活动状态"
      ></el-table-column>
      <el-table-column align="center" prop="limitPeopleNum" label="砍价方式">
        <template slot-scope="scope">
          {{
            scope.row.bargainirgType == 1
              ? scope.row.bargainirgPrice + "元"
              : scope.row.bargainirgRandomMin +
                "元" +
                "—" +
                scope.row.bargainirgRandomMax +
                "元"
          }}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleClickTab(row, type) {
      console.log(row);
      this.$router.push({
        name: "bargainingDetail", //活动 + `?id=${row.actId}&type=${type}`,
        query: {
          id: row.actId,
          type: type,
        },
    ...
    getData() {
      GetActBargainrgPageAPI(this.queryListData, "get").then((res) => {
        console.log(res, "res");
        if (res.code == 101) {
          this.CODE = 101;
          return;
