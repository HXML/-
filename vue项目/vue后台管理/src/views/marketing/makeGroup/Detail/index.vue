 ...
 <errorPage v-if="CODE === 101" />
  <div class="makeGroup" v-else>
    ...
    >
      ...
      <el-form-item label="有效时间：" prop="groupValidHour">
        <el-select
          v-model="formData.groupValidHour"
          placeholder="请选择有效时间"
        >
        ...
        <span v-if="formData.groupValidHour" class="f10 tips"
          >若设置{{ formData.groupValidHour }}小时, 用户开团后,需要在{{
            formData.groupValidHour
          }}小时内成团,超时则拼团失败</span
        >
      </el-form-item>
      ...
      <el-form-item label="拼团人数：" prop="groupManNum">
        <el-select v-model="formData.groupManNum" placeholder="请选择">
          <el-option label="3" :value="3"></el-option>
          <el-option label="2" :value="2"></el-option>
          <el-option label="不限制" :value="1"></el-option> </el-select
        >人
        <span class="tips">不限制为单人团</span>
      </el-form-item>
      <el-form-item label="商品限购：" prop="goodsBuyNum">
        <el-select v-model="formData.goodsBuyNum" placeholder="请选择">
          <el-option label="3" :value="3"></el-option>
          <el-option label="2" :value="2"></el-option>
          <el-option label="不限制" :value="0"></el-option> </el-select
        >件/人
      </el-form-item>
    ...
    >
      <el-radio-group v-model="GoodIndex" class="flex fw">
        ...
        >
          <span>{{ good.goodsName }}</span></el-radio
        >
      </el-radio-group>
...
export default {
  data() {
    return {
      ...
      formData: {
        ...
        goodsId: null, //商品id
        hostHour: 24, //预热时间 （小时）
        groupManNum: null, //限制拼团人数
        goodsBuyNum: null, //限制购买商品数
        skuGoods: [], //商品sku(通过上个方法查询出)  修改后直接丢进来就行
      },
      rules: {
        actName: [
        ...
        groupManNum: [
          { required: true, message: "请选择拼团人数", trigger: "change" },
        ],
        goodsBuyNum: [
  ...
  methods: {
    ...
    getAcInfor(id) {
      ...
      GetEditActGroupInfoAPI(data, "get").then((res) => {
    ...
    createActivity() {
      ...
      if (this.cheackState == "Edit") {
        ...
        EditGroupActAPI(formData, "put").then((res) => {
          console.log(res, "res");
          if (res.code == 1) {
            this.$message({
              duration: 1000,
        ...
        InsertActGroupAPI(formData, "post").then((res) => {
          console.log(res, "res");
          if (res.code == 1) {
            this.$message({
              duration: 1000,
    ...
    diaLogConfirm() {
      ...
      console.log(
        GoodsID,
        this.GoodIndex,
        this.goodsList[this.selectGoodIndex],
        "GoodsID"
      );
