<template>
  ...
  <div class="onSale br-8">
    <el-form class="flex fw">
      ...
      <el-form-item label="完成时间：" label-width="120px" prop="name">
        <el-date-picker  value-format="yyyy-MM-dd" v-model="pageData.timeQ" type="date" placeholder="选择日期时间"></el-date-picker>
        <span class="mr-8 ml-8">至</span>
        <el-date-picker  value-format="yyyy-MM-dd"  v-model="pageData.TimeH" type="date" placeholder="选择日期时间"></el-date-picker>
      </el-form-item>
    </el-form>
    ...
    <el-table ref="filterTable" :data="tableData" style="width: 100%;margin-top:20px;">
      <el-table-column prop="orderNo" min-width="120px" label="订单号">
      ...
      <el-table-column prop="pUserCommission" label="订单用户佣金">
        <template slot-scope="scope">
          <!-- <span>{{scope.row.number}}</span>
          <i class="ml-8 el-icon-edit"></i>-->
          <div class="flex aic">{{ scope.row.pUserCommission}}</div>
        </template>
      </el-table-column>
      <el-table-column prop="commission" label="订单平台佣金">
        <template slot-scope="scope">
          <!-- <span>{{scope.row.number}}</span>
          <i class="ml-8 el-icon-edit"></i>-->
          <div class="flex aic">{{ scope.row.commission}}</div>
        </template>
      </el-table-column>

      <el-table-column prop="totalPrice" label="实付金额">
        <template slot-scope="scope">
          <!-- <span>{{scope.row.number}}</span>
          <i class="ml-8 el-icon-edit"></i>-->
          <div class="flex aic">{{ scope.row.totalPrice}}</div>
        </template>
      </el-table-column>
      <el-table-column prop="receiptTime" label="完成时间"></el-table-column>
    </el-table>
...
<script>
import { OrdertRunNoListAPI } from "@/api/settlementAPI";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      //获取数据
      OrdertRunNoListAPI(this.pageData, "get")
        .then(res => {
          console.log(res);
