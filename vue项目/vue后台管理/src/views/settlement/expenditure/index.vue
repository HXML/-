  ...
  <div class="onSale br-8" v-else>
    <el-form class="flex fw">
      ...
      <el-form-item label="创建时间：" label-width="120px" prop="name">
        <el-date-picker class="w-164"  value-format="yyyy-MM-dd" v-model="pageData.timeQ" type="date" placeholder="选择日期时间"></el-date-picker>
        <span class="mr-8 ml-8">至</span>
        <el-date-picker class="w-164"  value-format="yyyy-MM-dd" v-model="pageData.TimeH" type="date" placeholder="选择日期时间"></el-date-picker>
      </el-form-item>
    </el-form>
    ...
    <el-table ref="filterTable" :data="tableData" style="width: 100%;margin-top:20px;">
      ...
      <el-table-column prop="incomeAmount" label="支出金额">
        <template slot-scope="scope">
          <div class="flex aic">
            <span>¥</span>
      ...
      <el-table-column prop="incomeTypeStr" label="支出类型">
        <template slot-scope="scope">
...
<script>
import { AccountRunNoListAPI } from "@/api/settlementAPI";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      //获取数据
      AccountRunNoListAPI(this.pageData, "get")
        .then(res => {
          console.log(res);
          if (res.code == 101) {
            this.CODE = 101;
