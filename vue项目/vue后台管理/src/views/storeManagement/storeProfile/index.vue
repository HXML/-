<template>
  ...
  <div class="storeProfile br-8" v-else>
    <el-form class="flex fw">
      <el-form-item label="时间：" label-width="100px" prop="name">
        <el-select
          class="w-164"
        ...
        >
          ...
          <el-option label="近三月" :value="4"> </el-option>
          <el-option label="近六月" :value="5"> </el-option>
          <el-option label="近一年" :value="6"> </el-option>
        </el-select>
      </el-form-item>
    ...
    >
      ...
      <el-table-column prop="salesAmount" label="门店销售额">
        <template slot-scope="scope" class="flex">
          <el-button
            ...
            type="text"
            size="small"
            >{{ scope.row.salesAmount }}</el-button
          >
        </template>
      </el-table-column>
      <el-table-column prop="pushMoney" label="门店提成">
        <template slot-scope="scope" class="flex">
          <el-button
            class="underline"
            @click="Details(scope.row)"
...
import { GetPickupSurveyPageAPI, PassPickupApplyAPI } from "@/api/shopStoreAPI";
export default {
  data() {
    return {
      indexMethod: 1,
  ...
  methods: {
    exportExcel() {
      ...
      this.$confirm("确认导出本页数据？", "提示", {
        ...
        XLSX.writeFile(wb, "门店概况.xlsx"); // 保存的文件名
      });
    },
    handleClicks(val) {
      //切换状态
    ...
    getListData() {
      GetPickupSurveyPageAPI(this.tablePagination, "get").then((res) => {
        console.log(res, "res");
        if (res.code == 101) {
          this.CODE = 101;
          return;
